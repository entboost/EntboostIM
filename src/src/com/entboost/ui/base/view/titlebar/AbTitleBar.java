 ...
 */
package com.entboost.ui.base.view.titlebar;
...
public class AbTitleBar extends LinearLayout {
	...
	public void ininTitleBar(Context context) {
		...
		titleTextLayout.setPadding(10,0, 0, 0);
		...
		titleTextLayout.addView(titleTextBtn, new LinearLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, 1));
		...
		LayoutParams layoutParamslogo = new LinearLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		layoutParamslogo.gravity = Gravity.CENTER_VERTICAL;
		layoutParamslogo.setMargins(25, 0, 25, 0);
		this.addView(logoView, layoutParamslogo);
		...
		rightLayout.setPadding(20, 10, 20, 10);
	...
	public void setTitleBarGravity(int gravity1, int gravity2) {
		...
		// if(D)Log4jLog.d(LONG_TAG, "测量布局的宽度："+leftWidth+","+rightWidth);
	...
	private Map<Integer, PopMenu> popMenus = new HashMap<Integer, PopMenu>();

	public PopMenu getPopMenus(int resId) {
		return popMenus.get(resId);
	}

	public void addRightImageButton(final int resId,
			final PopMenuConfig config, PopMenuItem... items) {
		rightLayout.setVisibility(View.VISIBLE);
		ImageButton img = new ImageButton(rightLayout.getContext());
		img.setImageResource(resId);
		img.setBackgroundDrawable(null);
		img.setPadding(20, 0, 20, 0);
		rightLayout.addView(img, layoutParamsWW);
		if (items != null && items.length != 0) {
			if (items.length == 1) {
				final PopMenuItem item = items[0];
				...
				PopMenu popMenu = new PopMenu(rightLayout.getContext(), config);
				popMenu.addItems(items);
				popMenus.put(resId, popMenu);
				img.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						if (config != null) {
							popMenus.get(resId).showAsDropDown(v,
									config.getShowAsDropDownYoff());
						} else {
							popMenus.get(resId).showAsDropDown(v);
						}
					}
