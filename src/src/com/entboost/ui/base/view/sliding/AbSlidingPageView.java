...
public class AbSlidingPageView extends ViewGroup {
	...
	/** 记录日志的标记. */
	private static String TAG = AbSlidingPageView.class.getSimpleName();
	private static String LONG_TAG = AbSlidingPageView.class.getName();
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if(D) Log4jLog.d(LONG_TAG, "--onLayout--:"+width);
			if(i==0){
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(D) Log4jLog.d(LONG_TAG, "--onMeasure--");
	...
	public void showNext() {
		...
		if(D) Log4jLog.d(LONG_TAG, "--showNext--:"+getScrollX()+" dx "+(getChildAt(1).getWidth()-2*nextViewOffset));
	...
	public void showPrevious() {
		...
		if(D) Log4jLog.d(LONG_TAG, "--showPrevious--:"+getScrollX()+" dx -"+getScrollX());
		//滚动回onlayout的0的位置
