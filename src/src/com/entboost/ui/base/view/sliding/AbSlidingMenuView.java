...
public class AbSlidingMenuView extends ViewGroup {
	...
	/** The tag. */
	private static String TAG = AbSlidingMenuView.class.getSimpleName();
	private static String LONG_TAG = AbSlidingMenuView.class.getName();
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			if(D) Log4jLog.d(LONG_TAG, "--dispatchTouchEvent ACTION_DOWN--");
			...
			if (mTouchState == TOUCH_STATE_RESTART) {
				...
				if(D) Log4jLog.d(LONG_TAG, "--dispatchTouchEvent ACTION_DOWN return false--");
				return false;
			}
			break;
		case MotionEvent.ACTION_MOVE:
			if(D) Log4jLog.d(LONG_TAG, "--dispatchTouchEvent ACTION_MOVE--");
			mVelocityTracker.computeCurrentVelocity(1000,ViewConfiguration.getMaximumFlingVelocity());
			if (mScrollState == SCROLL_STATE_ALLOW && getWidth() - (int) ev.getX() < mWidth) {
				if(D) Log4jLog.d(LONG_TAG, "--dispatchTouchEvent ACTION_MOVE return true--");
				return true;
			}
			break;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		switch (ev.getAction()) {
		case MotionEvent.ACTION_DOWN:
			if(D) Log4jLog.d(LONG_TAG, "--onInterceptTouchEvent ACTION_DOWN--");
			mTouchState = mScroller.isFinished() ? TOUCH_STATE_RESTART : TOUCH_STATE_SCROLLING;
			if (mTouchState==TOUCH_STATE_SCROLLING) {
				if(D) Log4jLog.d(LONG_TAG, "--onInterceptTouchEvent ACTION_DOWN return false--");
				return false;
			}
			break;

		case MotionEvent.ACTION_MOVE:
			if(D) Log4jLog.d(LONG_TAG, "--onInterceptTouchEvent ACTION_MOVE--");
			mOnClick = false;
			mVelocityTracker.computeCurrentVelocity(1000,ViewConfiguration.getMaximumFlingVelocity());
			if (mScrollState == SCROLL_STATE_ALLOW && Math.abs(mVelocityTracker.getXVelocity()) > 200) {
				if(D) Log4jLog.d(LONG_TAG, "--onInterceptTouchEvent ACTION_MOVE return true--");
				return true;
			}
			break;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			if(D) Log4jLog.d(LONG_TAG, "--onTouchEvent ACTION_DOWN--");
			mTouchState = mScroller.isFinished() ? TOUCH_STATE_RESTART : TOUCH_STATE_SCROLLING;
			if (mTouchState==TOUCH_STATE_SCROLLING) {
				if(D) Log4jLog.d(LONG_TAG, "--onTouchEvent ACTION_DOWN return false--");
				return false;
			}
			break;

		case MotionEvent.ACTION_MOVE:
			if(D) Log4jLog.d(LONG_TAG, "--onTouchEvent ACTION_MOVE--");
			mVelocityTracker.computeCurrentVelocity(1000,ViewConfiguration.getMaximumFlingVelocity());
