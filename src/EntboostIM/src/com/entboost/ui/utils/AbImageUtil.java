...
import com.entboost.ui.utils.dct.FDCT;
import com.entboost.utils.AbMathUtil;
...
public class AbImageUtil {
	...
	/** The tag. */
	private static String TAG = AbImageUtil.class.getSimpleName();
	private static String LONG_TAG = AbImageUtil.class.getName();
	...
	public static Bitmap getBitmapFormURL(String imageUrl, int type,
		...
		try {
			...
			if (D)
				Log4jLog.d(LONG_TAG, "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	public static Bitmap scaleImg(File file, int newWidth, int newHeight) {
		...
		// 使图片不抖动
		opts.inDither = false;
		// if(D) Log4jLog.d(LONG_TAG, "将缩放图片:"+file.getPath());
		resizeBmp = BitmapFactory.decodeFile(file.getPath(), opts);
		// 缩小或者放大
		if (resizeBmp != null && scale != 1) {
			resizeBmp = scaleImg(resizeBmp, scale);
		}
		// if(D) Log4jLog.d(LONG_TAG, "缩放图片结果:"+resizeBmp);
		return resizeBmp;
	}
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					if (D)
						Log4jLog.d(LONG_TAG, "Bitmap释放" + bitmap.toString());
					bitmap.recycle();
				}
			} catch (Exception e) {
			}
	...
	public static String getHashCode(Bitmap bitmap) {
		...
		Log4jLog.i(LONG_TAG, "将图片缩小到8x8的尺寸:" + width + "*" + height);
	...
	public static String getDCTHashCode(Bitmap bitmap) {
		...
		int width = temp.getWidth();
		int height = temp.getHeight();
		Log4jLog.i(LONG_TAG, "将图片缩小到32x32的尺寸:" + width + "*" + height);
